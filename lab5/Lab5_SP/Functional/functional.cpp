...
void LoadPlugins(bool lang)
{
	...
	HANDLE hFile = FindFirstFile("Plugins\\*.dll", &file);//ищет каталог файла или подкаталог, название которого соответствует указанному имени файла
...
void ShowInfo(bool lang)
{
	...
	for (int i = 0; i < pluginsCount; ++i)
	{
		...
		PI(buffer, 512, &bufdword);
		if (!lang)
			printf("Автор: Андрей Гончаренко");
		else
			printf("Author: Andrey Goncharenko");
	}
}
